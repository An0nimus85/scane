...
# Основная часть скрипта с argparse для обработки аргументов командной строки
def main():
    parser = argparse.ArgumentParser(
        description='Скрипт для проверки регистрации номера телефона на нескольких сайтах.')
    parser.add_argument('phone_number', type=str, help='Номер телефона для проверки')
    ...
    for site in sites:
        ...
        if 'phone' in site['form_data']:
            site['form_data']['phone'] = phone_number
        if 'phone_number' in site['form_data']:
            site['form_data']['phone_number'] = phone_number
